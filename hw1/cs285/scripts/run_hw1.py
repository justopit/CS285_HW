...
def run_training_loop(params):
    ...
    print('ob_dim and ac_dim', ob_dim, ac_dim)
    ...
    for itr in range(params['n_iter']):
            ...
            if params['do_dagger']:
                ...
                for path in paths:
                    ...
                    for i in range(len(path['observation'])):                        
                        ac = expert_policy(torch.tensor(path['observation'][i], dtype=torch.float))
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          train_bs_idxs = np.random.permutation(np.arange(len(replay_buffer)))[:params['train_batch_size']]
          ob_batch, ac_batch = torch.tensor(replay_buffer.obs[train_bs_idxs], dtype=torch.float), torch.tensor(replay_buffer.acs[train_bs_idxs], dtype=torch.float)
