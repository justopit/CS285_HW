...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if step >= config["training_starts"]:
            ...
            update_info =  agent.update(ptu.from_numpy(batch['observations']),ptu.from_numpy(batch['actions']), ptu.from_numpy(batch['rewards']), ptu.from_numpy(batch['next_observations']), ptu.from_numpy(batch['dones']), step=step)
